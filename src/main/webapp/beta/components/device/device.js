...
.controller("DeviceController", function($scope, $rootScope, $location, $routeParams, HeaderService, ProjectService, DeviceService) { 
	console.log("Device Controller executing...");
	...
	console.log("  Device controller retrieves current project " + _currentProject);
	
	var _params = [];
	var _deviceId = $routeParams.deviceId;
	console.log("  Device controller retrieves device id " + _deviceId);

	var _currentDevice = DeviceService.getCurrentDevice();
	console.log("  Device controller sets current device " + _currentDevice);
	...
	$scope.$on('currentProject_change', function() {
		...
		console.log("Device currentProject_change received ");
		console.log(_currentProject);
		
		$scope.project = _currentProject;
		if (_currentDevice != null && _currentDevice.id == null && _deviceId != null && _deviceId != undefined) {
			$scope.loading = false;
	...
	$scope.saveDevice = function(device) {
		...
		console.log("  Device controller attempts to save a device on project " + _currentProject.id);
		
		if (device != null && _currentProject != null) {
			
			device.projectId = _currentProject.id;
			
			console.log("  Device controller says that the device to save is: ");
			...
			if (_update == true) {
				...
				.then(function(device) { 
					console.log("  Device controller updates device " + device.name + " (" + device.id + ")");
				
					// HEY WHAT IS THIS DOING HERE. SHOULDNT IT BE HEADER SERVICE INSTEAD OF PROJECT SERVICE?
					/*ProjectService.list()
				  .then(function(data) { 
			
						console.log("  Retrieved " + data.length + " projects");
						HeaderService.setProjects(data);
					...
					HeaderService.updateDevice(_currentProject.id, device);
					$rootScope.$broadcast('projectList_change');
					DeviceService.removeCurrentDevice();
					$location.path('monitor/' + _currentProject.id);
				...
				.then(function(device) { 
					...
					console.log("  Device controller saves device " + device.name + " (" + device.id + ")");
				  ...
				  .then(function(data) { 
					...
					HeaderService.addDevice(_currentProject.id, device);
					$rootScope.$broadcast('projectList_change');
					DeviceService.removeCurrentDevice();
					$location.path('monitor/' + _currentProject.id);
